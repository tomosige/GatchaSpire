...
namespace GatchaSpire.Gameplay.Battle
{
    ...
    public class BattleManagerTest : TestExclusiveBase
    {
        ...
        private IEnumerator RunAllTestsSequentially()
        {
            ...
            yield return StartCoroutine(TestBattleEvents());
        ...
        private IEnumerator TestBattleManagerInitialization()
        {
            ...
            AssertTest(battleManager.CurrentBattleState == BattleState.Idle, "初期状態がIdleであること");
            AssertTest(battleManager.BattleTimer == 0f, "初期戦闘時間が0であること");
            AssertTest(!battleManager.IsBattleActive, "初期状態で戦闘が非アクティブであること");
            AssertTest(battleManager.PlayerCombatCharacters.Count == 0, "初期プレイヤー戦闘キャラクター数が0であること");
            AssertTest(battleManager.EnemyCombatCharacters.Count == 0, "初期敵戦闘キャラクター数が0であること");
        ...
        private IEnumerator TestBattleSetupValidation()
        {
            ...
            if (testEnemyCharacters.Count > 0)
            {
                ...
                AssertTest(validSetup.EnemyCharacters.Count > 0, "敵キャラクターが設定されていること");
        ...
        private IEnumerator TestVictoryConditions()
        {
            ...
            // 短時間制限戦闘でタイムアウトテスト
            SetupTestBattle();
            var timeoutSetup = CreateValidBattleSetup();
            timeoutSetup.TimeLimit = 0.5f; // 0.5秒制限

            battleManager.StartBattle(timeoutSetup);

            // タイムアウトまで待機
            yield return new WaitForSeconds(1f);

            AssertTest(battleEndedEventFired, "時間制限により戦闘が終了すること");
            if (lastBattleResult != null)
            {
                AssertTest(lastBattleResult.BattleOutcome == BattleOutcome.Draw, "時間切れで引き分けになること");
                AssertTest(lastBattleResult.BattleDuration > 0f, "戦闘時間が記録されること");
            }
        ...
        /// 戦闘イベントテスト
        /// </summary>
        private IEnumerator TestBattleEvents()
        {
            LogDebug("戦闘イベントテスト開始");

            if (testPlayerCharacters.Count == 0 || testEnemyCharacters.Count == 0)
            {
                LogTestResult("テストキャラクター不足のため戦闘イベントテストをスキップ");
                yield return new WaitForSeconds(0.1f);
                yield break;
            }
            ...
            // イベントハンドラー登録
            battleManager.OnBattleStateChanged += (oldState, newState) => stateChangedFired = true;
            battleManager.OnBattleEnded += (result) => battleEndedFired = true;
            ...
            yield return new WaitForSeconds(0.1f);
            AssertTest(stateChangedFired, "戦闘開始時に状態変更イベントが発火すること");

            battleManager.ForceEndBattle();
            yield return new WaitForSeconds(0.1f);
            AssertTest(battleEndedFired, "戦闘終了時に終了イベントが発火すること");

            LogTestResult("戦闘イベントテスト完了");
            yield return new WaitForSeconds(0.1f);
        }
        ...
        /// テスト戦闘強制終了
        /// </summary>
        [ContextMenu("Force End Test Battle")]
        public void ForceEndTestBattle()
        {
            if (battleManager != null)
            {
                battleManager.ForceEndBattle();
                ReportInfo("テスト戦闘を強制終了しました");
            }
        }
    }
}